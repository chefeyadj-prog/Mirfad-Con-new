...
const Retroactive: React.FC = () => {
  ...
  const formatExcelDate = (value: any) => {
    ...
    try {
      ...
      return /^\d{4}-\d{2}-\d{2}/.test(strDate) ? strDate.split(' ')[0] : strDate;
  ...
  const processFile = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (mapped.length === 0) setErrorMsg('لا توجد بيانات صالحة في الملف');
        else setPreviewData(mapped);
      } catch (err: any) {
  ...
  const saveToDatabase = async () => {
    ...
    try {
      ...
      await logAction(user, 'create', 'الأثر الرجعي', `رفع ملف مشتريات بأثر رجعي يحتوي على ${previewData.length} سجل`);
      setSuccessMsg(`تم استيراد ${previewData.length} فاتورة بنجاح.`);
      setPreviewData([]);
    } catch (err: any) {
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-2xl border border-slate-100 shadow-sm gap-4">
        ...
        <div className="flex gap-3">
           <button onClick={downloadTemplate} className="px-4 py-2 text-slate-600 bg-slate-50 hover:bg-slate-100 rounded-lg flex items-center gap-2 transition-colors border border-slate-200">
             <Download size={18} /><span>تحميل النموذج</span>
           </button>
           <button onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg flex items-center gap-2 shadow-md transition-all disabled:opacity-50">
              {isUploading ? <Loader2 className="animate-spin" size={18} /> : <Upload size={18} />}<span>رفع الملف</span>
           </button>
      ...
      {successMsg && (
        <div className="bg-green-50 border border-green-200 text-green-700 p-4 rounded-xl flex items-center gap-3 animate-fade-in shadow-sm">
          <CheckCircle2 size={24} className="shrink-0" /><p className="font-bold">{successMsg}</p>
        </div>
      )}
      ...
      {errorMsg && (
        <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-xl flex items-center gap-3 animate-fade-in shadow-sm">
          ...
          <div><p className="font-bold">فشل في معالجة البيانات</p><p className="text-sm opacity-90">{errorMsg}</p></div>
        </div>
      )}
      ...
      {previewData.length > 0 ? (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden animate-scale-in">
          <div className="p-4 bg-slate-50 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
             <h3 className="font-bold text-slate-700 flex items-center gap-2 shrink-0"><Table size={18} className="text-indigo-500" />معاينة البيانات ({previewData.length} سجل)</h3>
             <div className="flex items-center gap-3 bg-white p-1 rounded-xl border border-slate-200 shadow-sm shrink-0">
                <span className="text-xs font-bold text-slate-400 mr-3">طريقة الدفع:</span>
                <button onClick={() => setGlobalPaymentMethod('credit')} className={`px-4 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${globalPaymentMethod === 'credit' ? 'bg-orange-600 text-white shadow-md' : 'text-slate-500'}`}><Clock size={14} /> آجل</button>
                <button onClick={() => setGlobalPaymentMethod('cash')} className={`px-4 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${globalPaymentMethod === 'cash' ? 'bg-green-600 text-white shadow-md' : 'text-slate-500'}`}><Banknote size={14} /> نقدي</button>
                <button onClick={() => setGlobalPaymentMethod('transfer')} className={`px-4 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 transition-all ${globalPaymentMethod === 'transfer' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500'}`}><CreditCard size={14} /> حوالة</button>
             </div>
             <div className="flex gap-2 mr-auto">
                <button onClick={() => setPreviewData([])} className="px-4 py-2 text-red-600 hover:bg-red-50 rounded-lg font-medium">إلغاء</button>
                <button onClick={saveToDatabase} disabled={isUploading} className="bg-green-600 hover:bg-green-700 text-white px-8 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-md disabled:opacity-50">
                {isUploading ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />}تأكيد الحفظ النهائي</button>
             </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-right text-sm">
              <thead className="bg-slate-50 text-slate-500 font-bold"><tr className="border-b border-slate-100"><th className="p-4">المورد</th><th className="p-4">التاريخ</th><th className="p-4">رقم الفاتورة</th><th className="p-4">قبل الضريبة</th><th className="p-4">الضريبة</th><th className="p-4">الاجمالي</th></tr></thead>
              <tbody className="divide-y divide-slate-100">{previewData.map((row, idx) => (<tr key={idx} className="hover:bg-slate-50 transition-colors"><td className="p-4 font-bold text-slate-800">{row.supplier}</td><td className="p-4 text-slate-600 font-mono">{row.date}</td><td className="p-4 text-slate-500 font-mono">{row.invoiceNo}</td><td className="p-4 text-slate-600">{row.netAmount.toLocaleString()}</td><td className="p-4 text-slate-600">{row.vatAmount.toLocaleString()}</td><td className="p-4 font-bold text-indigo-600">{row.totalAmount.toLocaleString()}</td></tr>))}</tbody>
            </table>
          </div>
        </div>
      ) : !successMsg && (
        <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border-2 border-dashed border-slate-200 text-slate-400">
           <div className="w-24 h-24 bg-indigo-50 text-indigo-400 rounded-full flex items-center justify-center mb-6 shadow-inner"><FileSpreadsheet size={48} /></div>
