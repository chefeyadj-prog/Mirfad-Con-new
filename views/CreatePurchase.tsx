...
const CreatePurchase: React.FC = () => {
  ...
  // =============== A LOGIC: init with unique ID ===============
  const [items, setItems] = useState<InvoiceItem[]>([
    { id: generateId(), code: '', description: '', quantity: 1, unitPrice: 0 },
  ]);

  // =============== A LOGIC: edit mode re-generate item IDs ===============
  useEffect(() => {
    if (id && purchases.length > 0) {
      ...
      if (purchaseToEdit) {
        ...
        if (purchaseToEdit.items && purchaseToEdit.items.length > 0) {
          setItems(purchaseToEdit.items.map((i) => ({ ...i, id: generateId() })));
        }
    ...
    if (invoiceNumber && supplierId) {
      ...
      if (currentSupplier) {
        const exists = purchases.some((p) => {
          ...
          return (
            (p.invoiceNumber === invoiceNumber || p.id === invoiceNumber) &&
            p.partyName === currentSupplier.name
          );
        });
  ...
  // =============== A LOGIC: add item uses generateId ===============
  const handleAddItem = () =>
    setItems([...items, { id: generateId(), code: '', description: '', quantity: 1, unitPrice: 0 }]);

  const handleRemoveItem = (id: string) => {
    if (items.length > 1) setItems(items.filter((item) => item.id !== id));
  };

  // =============== A LOGIC: safe updateItem (row-specific + sanitize numeric + autofill) ===============
  const updateItem = (id: string, field: keyof InvoiceItem, value: any) => {
    setItems((prevItems) => {
      ...
      if (field === 'quantity' || field === 'unitPrice') {
        ...
        processedValue = cleaned === '' ? 0 : Number(cleaned);
      }
      ...
      return prevItems.map((item) => {
        ...
        if (field === 'code') {
          ...
          if (product) {
            ...
            newItem.unitPrice = product.cost;
          }
        }
  ...
  // =============== A LOGIC: analyzeFiles keeps + uses generateId ===============
    ...
    try {
      ...
      if (analysisResult) {
        ...
        if (analysisResult.currency)
          setCurrency(analysisResult.currency.toUpperCase().includes('USD') ? 'USD' : 'SAR');
        ...
        if (analysisResult.supplierName) {
          ...
          const matchedSupplier = suppliers.find((s) => {
            ...
            return (
              storedNameNorm === detectedNameNorm ||
              (storedNameNorm.length > 3 && detectedNameNorm.includes(storedNameNorm)) ||
              (detectedNameNorm.length > 3 && storedNameNorm.includes(detectedNameNorm))
            );
          });
  ...
  // =============== A LOGIC: Excel import with cleaning + generateId ===============
        ...
        if (jsonData.length > 0) {
          setItems(
            (jsonData as any[]).map((row: any) => ({
              ...
              quantity: Number(String(row['الكمية'] || row['Qty'] || row['Quantity'] || row['العدد'] || 1).replace(/[^\d.]/g, '')),
              unitPrice: Number(String(row['السعر'] || row['Price'] || row['Cost'] || row['التكلفة'] || 0).replace(/[^\d.]/g, '')),
            }))
          );
        }
      };
        ...
        if (!skipInventory) {
          for (const item of items) {
            const product = inventoryProducts.find(
              (p) => (item.code && p.sku === item.code) || p.name === item.description
            );
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <button
            ...
            className="p-2 bg-white border border-slate-200 rounded-full text-slate-500 hover:bg-slate-50 transition-colors shadow-sm"
          >
            <ArrowRight size={20} />
          </button>
          ...
          <div className="flex justify-between items-start mb-4">
            ...
            {isAnalyzing && (
              <div className="text-indigo-600 text-sm flex items-center gap-2">
                <Loader2 className="animate-spin" size={16} /> جاري التحليل...
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full p-2 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 ${
                duplicateWarning ? 'border-orange-300 focus:ring-orange-200' : 'border-slate-300'
              }`}
              value={invoiceNumber}
                  ...
                  filteredSuppliers.map((s) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-slate-500">
                        {s.phone} {s.taxNumber && `| ضريبي: ${s.taxNumber}`}
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="p-3 text-center text-slate-500 text-sm">لا يوجد نتائج.</div>
                )}
              </div>
            )}
          </div>
